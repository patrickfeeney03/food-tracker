...
<div class="min-h-dvh bg-[var(--app-canvas)] sm:px-4 sm:py-6">
  ...
  >
    ...
    <form method="POST" class="flex flex-1 flex-col px-3 pb-28 sm:px-8">
        ...
        <fieldset class="mb-6">
          ...
          >
            {#each data.portionOptions as option (option.kind)}
              <label
                class={[
                  "flex min-h-10 cursor-pointer items-center justify-center rounded-full border px-2 text-center text-[12px] font-bold transition",
      ...
      <div
        class="fixed inset-x-0 bottom-0 z-10 bg-[var(--app-surface)] px-3 py-3
